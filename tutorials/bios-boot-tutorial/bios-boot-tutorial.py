...
def msigReplaceSystem():
    run(args.cleos + 'push action vexcore buyrambytes' + jsonArg(['vexcore', accounts[0]['name'], 200000]) + '-p vexcore')
    sleep(1)
...
def stepSetSystemContract():
    ...
    # ACTION_RETURN_VALUE
    retry(args.cleos + 'push action vexcore activate \'["c3a6138c5061cf291310887c0b5c71fcaffeab90d5deb50d3b9e687cead45071"]\' -p vexcore@active')
    # CONFIGURABLE_WASM_LIMITS2
    retry(args.cleos + 'push action vexcore activate \'["d528b9f6e9693f45ed277af93474fd473ce7d831dae2180cca35d907bd10cb40"]\' -p vexcore@active')
    # BLOCKCHAIN_PARAMETERS
    retry(args.cleos + 'push action vexcore activate \'["5443fcf88330c586bc0e5f3dee10e7f63c76c00249c87fe4fbf7f38c082006b4"]\' -p vexcore@active')
    # GET_SENDER
    retry(args.cleos + 'push action vexcore activate \'["f0af56d2c5a48d60a4a5b5c903edfb7db3a736a94ed589d0b797df33ff9d3e1d"]\' -p vexcore@active')
    # FORWARD_SETCODE
    retry(args.cleos + 'push action vexcore activate \'["2652f5f96006294109b3dd0bbde63693f55324af452b799ee137a81a905eed25"]\' -p vexcore@active')
    # ONLY_BILL_FIRST_AUTHORIZER
    retry(args.cleos + 'push action vexcore activate \'["8ba52fe7a3956c5cd3a656a3174b931d3bb2abb45578befc59f283ecd816a405"]\' -p vexcore@active')
    # RESTRICT_ACTION_TO_SELF
    retry(args.cleos + 'push action vexcore activate \'["ad9e3d8f650687709fd68f4b90b41f7d825a365b02c23a636cef88ac2ac00c43"]\' -p vexcore@active')
    # DISALLOW_EMPTY_PRODUCER_SCHEDULE
    retry(args.cleos + 'push action vexcore activate \'["68dcaa34c0517d19666e6b33add67351d8c5f69e999ca1e37931bc410a297428"]\' -p vexcore@active')
    # FIX_LINKAUTH_RESTRICTION
    retry(args.cleos + 'push action vexcore activate \'["e0fb64b1085cc5538970158d05a009c24e276fb94e1a0bf6a528b48fbc4ff526"]\' -p vexcore@active')
    # REPLACE_DEFERRED
    retry(args.cleos + 'push action vexcore activate \'["ef43112c6543b88db2283a2e077278c315ae2c84719a8b25f25cc88565fbea99"]\' -p vexcore@active')
    # NO_DUPLICATE_DEFERRED_ID
    retry(args.cleos + 'push action vexcore activate \'["4a90c00d55454dc5b059055ca213579c6ea856967712a56017487886a4d4cc0f"]\' -p vexcore@active')
    # ONLY_LINK_TO_EXISTING_PERMISSION
    retry(args.cleos + 'push action vexcore activate \'["1a99a59d87e06e09ec5b028a9cbb7749b4a5ad8819004365d02dc4379a8b7241"]\' -p vexcore@active')
    # RAM_RESTRICTIONS
    retry(args.cleos + 'push action vexcore activate \'["4e7bf348da00a945489b2a681749eb56f5de00b900014e137ddae39f48f69d67"]\' -p vexcore@active')
    # WEBAUTHN_KEY
    retry(args.cleos + 'push action vexcore activate \'["4fca8bd82bbd181e714e283f83e1b45d95ca5af40fb89ad3977b653c448f78c2"]\' -p vexcore@active')
    # WTMSIG_BLOCK_SIGNATURES
    retry(args.cleos + 'push action vexcore activate \'["299dcb6af692324b899b39f16d5a530a33062804e41f09dc97e9f156b4476707"]\' -p vexcore@active')
    # GET_CODE_HASH
    retry(args.cleos + 'push action vexcore activate \'["bcd2a26394b36614fd4894241d3c451ab0f6fd110958c3423073621a70826e99"]\' -p vexcore@active')
    # GET_BLOCK_NUM
    retry(args.cleos + 'push action vexcore activate \'["35c2186cc36f7bb4aeaf4487b36e57039ccf45a9136aa856a5d569ecca55ef2b"]\' -p vexcore@active')
    # CRYPTO_PRIMITIVES
    retry(args.cleos + 'push action vexcore activate \'["6bcb40a24e49c26d0a60513b6aeb8551d264e4717f306b81a37a5afb3b47cedc"]\' -p vexcore@active')
    # BLS_PRIMITIVES2
    retry(args.cleos + 'push action vexcore activate \'["63320dd4a58212e4d32d1f58926b73ca33a247326c2a5e9fd39268d2384e011a"]\' -p vexcore@active')
    # DISABLE_DEFERRED_TRXS_STAGE_1 - DISALLOW NEW DEFERRED TRANSACTIONS
    retry(args.cleos + 'push action vexcore activate \'["fce57d2331667353a0eac6b4209b67b843a7262a848af0a49a6e2fa9f6584eb4"]\' -p vexcore@active')
    ...
    retry(args.cleos + 'push action vexcore activate \'["09e86cb0accf8d81c9e85d34bea4b925ae936626d00c984e4691186891f5bc16"]\' -p vexcore@active')
    sleep(1)
...
def stepInitSystemContract():
    run(args.cleos + 'push action vexcore init' + jsonArg(['0', '4,' + args.symbol]) + '-p vexcore@active')
    sleep(1)
...
parser.add_argument('--public-key', metavar='', help="VEXANIUM Public Key", default='VEX8Znrtgwt8TfpmbVpTKvA2oB8Nqey625CLN8bCN3TEbgx86Dsvr', dest="public_key")

...
void system_contract::runrex( uint16_t max )
{
   ...
   /// transfer from vex.names to vex.rex
...
void system_contract::channel_to_rex( const name& from, const asset& amount )
{
   ...
   if ( rex_available() ) {
         ...
         INLINE_ACTION_SENDER(eosio::token, transfer)( token_account, { from, active_permission },
            { from, rex_account, amount, std::string("transfer from ") + name{from}.to_string() + " to vex.rex"} );
      }
#endif
}
...
{
   /**
    * If CORE_SYMBOL is (VEX,4), maximum supply is 10^10 tokens (10 billion tokens), i.e., maximum amount

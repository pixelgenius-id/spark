...
class Cluster(object):
    ...
    def __init__(self, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899
        ...
        host: vex server host
        port: vex server port
        walletHost: vex wallet host
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on vex server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on vex node port: %d" % (node.port))
                return False
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse vexanium private keys from cluster start files.")
            return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission vexcore@active"
            ...
            # wait for block production handover (essentially a block produced by anyone but vexcore).
        ...
        systemAccounts = ['vex.bpay', 'vex.msig', 'vex.names', 'vex.ram', 'vex.ramfee', 'vex.saving', 'vex.stake', 'vex.token', 'vex.vpay', 'vex.wrap']
    ...
    # Kills a percentange of Vex instances starting from the tail and update eosInstanceInfos state

...
class cluster_generator:
    ...
    def parseArgs(self, args):
        ...
        parser.add_argument('--config-dir', type=Path, help='directory containing configuration files such as config.ini', default=Path('etc') / 'vexcore')
    ...
    def bind_nodes(self):
        ...
        for account, node in zip(accounts, self.network.nodes.values()):
            ...
            if is_bios:
                node.keys.append(KeyStrings('VEX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV',
                                            '5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3'))
                node.producers.append('vexcore')
            else:

...
BOOST_FIXTURE_TEST_CASE( get_account, validating_tester ) try {
   ...
   // test link authority to vex.any
   link_authority(name("alice"_n), name("bob"_n), name("vex.any"_n), name("foo"_n));
   produce_block();
   result = get_account_full(plugin, p, fc::time_point::maximum());
   check_result_basic(result, name("alice"_n), false);
   // active permission should no longer have linked auth, as vex.any replaces it

...
def cleos_sign_test():
    ...
    cmd = ['./programs/cleos/cleos', '--print-request', '--no-auto-keosd', 'sign', '-c', chain, '--public-key', 'VEX8Dq1KosJ9PMn1vKQK3TbiihgfUiDBUsz471xaCE6eYUssPB1KY', packed_trx]
    outs=None
    errs=None
    try:
...
def cleos_abi_file_test():
    ...
    token_abi_file_arg = 'vex.token' + ':' + token_abi_path
    system_abi_file_arg = 'vexcore' + ':' + system_abi_path
    ...
    invalid_abi_arg = 'vex.token' + ' ' + token_abi_path
    ...
    unpacked_action_data = """{
        ...
        "owner": {
          "threshold": 1,
          "keys": [{
              "key": "VEX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
              "weight": 1
            }
          ],
          "accounts": [],
    ...
    outs, errs = processCleosCommand(cmd)
    assert(b'"creator": "vexcore"' in outs)
...
def abi_file_with_nodeos_test():
    ...
    try:
        ...
        time.sleep(5)
        node.waitForBlock(1)
        accountNames = ["vexcore", "vex.token", "alice", "bob"]
        accounts = []
        for name in accountNames:
            account = Account(name)
            account.ownerPrivateKey = account.activePrivateKey = "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"
            account.ownerPublicKey = account.activePublicKey = "VEX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
            accounts.append(account)
        walletMgr.create('vexcore', [accounts[0]])
        ...
        account = 'vex.token'
        action = 'create'
        data = '{"issuer":"vex.token","maximum_supply":"100000.0000 SYS","can_freeze":"0","can_recall":"0","can_whitelist":"0"}'
        permission = '--permission vex.token@active'
        node.pushMessage(account, action, data, permission)
        action = 'issue'
        data = '{"from":"vex.token","to":"vex.token","quantity":"100000.0000 SYS","memo":"issue"}'
        node.pushMessage(account, action, data, permission)
        ...
        node.processCleosCmd('set abi vex.token ' + malicious_token_abi_path, 'set malicious vex.token abi', returnType=ReturnType.raw)
        ...
        balance = node.getCurrencyBalance('vex.token', 'alice')

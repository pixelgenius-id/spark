...
class memory {
      ...
      static_assert(sizeof(control_block) <= wcb_allowance, "VEX VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      static_assert(EOS_VM_OC_CONTROL_BLOCK_OFFSET + cb_offset == 0, "VEX VM OC control block offset has slid out of place somehow");
      static_assert(stride == EOS_VM_OC_MEMORY_STRIDE, "VEX VM OC memory stride has slid out of place somehow");

...
string default_wallet_url = "unix://" + (determine_home_directory() / "vex-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
struct create_account_subcommand {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         ...
         createAccount->add_option("--buy-ram", buy_ram_vex,
      ...
      createAccount->callback([this] {
            ...
            if (!simple) {
               EOSC_ASSERT( buy_ram_vex.size() || buy_ram_bytes_in_kbytes || buy_ram_bytes, "ERROR: One of --buy-ram, --buy-ram-kbytes or --buy-ram-bytes should have non-zero value" );
               EOSC_ASSERT( !buy_ram_bytes_in_kbytes || !buy_ram_bytes, "ERROR: --buy-ram-kbytes and --buy-ram-bytes cannot be set at the same time" );
               action buyram = !buy_ram_vex.empty() ? create_buyram(name(creator), name(account_name), to_asset(buy_ram_vex))
...
struct activate_subcommand {
   ...
   std::string account_str = "vexcore";
   std::string permission_str = "vexcore";
   ...
   activate_subcommand(CLI::App* actionRoot) {
      ...
      activate->add_option("-a,--account", account_str, localized("The contract account name, default is vexcore"));
      activate->add_option("-p,--permission", permission_str, localized("The permission level to authorize, default is vexcore"));
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      std::cout << indent << "vex.any: " << std::endl;
...
//                                ->check(CLI::ExistingFile);
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send vex.system contract action to the blockchain."));
